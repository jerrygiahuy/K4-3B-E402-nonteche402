...
- Video tham chiếu: [luồng prototype CP2](https://drive.google.com/file/d/1I0A3aqYq2b-DOlYymKcN5ukSJNM7rZzV/view?usp=share_link) và [thao tác CP3](https://drive.google.com/file/d/1xoo4eIkc1Bh76xgtwR7V0_t-FGsvvhZZ/view?usp=sharing). Video CP3 cho thấy tương tác trên bản web nhưng phản hồi quan sát được giống nhánh quy tắc. Video `Recording 2026-09-19 014920.mp4` nhóm trưởng cung cấp sau đó cho thấy một lượt có nhãn `[AI Thật - Gemini API]`; DevTools Network hiển thị request `generateContent` tới `generativelanguage.googleapis.com` với HTTP `200 OK`. Khóa API không xuất hiện trong khung hình. Video này chứng minh một lượt gọi Gemini thành công, không chứng minh chất lượng toàn bộ 22 case; cần tải video lên Drive công khai và thêm link trước khi nộp cuối.
...
| [HAX G2 — Nói rõ giới hạn độ tin cậy](https://www.microsoft.com/en-us/haxtoolkit/guideline/make-clear-how-well-the-system-can-do-what-it-can-do/) | Phản hồi quy tắc được gắn nhãn riêng; phản hồi Gemini có nhãn AI thật. | Video API thật cho thấy nhãn Gemini và HTTP 200. Chưa có cảnh báo khi nguồn không đủ chắc. |
...
- **Trạng thái kiểm thử:** Đã có bằng chứng cho một lời gọi Gemini thật thành công: video `Recording 2026-09-19 014920.mp4` hiển thị nhãn AI thật và request `generateContent` nhận HTTP 200. Chưa có tỷ lệ pass/fail trên 22 case. `codebase/eval/eval_results.json` và `eval_report.md` ghi 22/22, nhưng `run_eval.py` và `run_eval.js` chỉ đọc trường `expected_behavior` của test case, không gọi AI/prototype và không chấm phản hồi thực tế. Vì vậy 22/22 chỉ phản ánh kiểm tra dữ liệu kỳ vọng trong script, không chứng minh đạt quality bar. Cần Hiệp chạy lại bộ test trên đầu ra thực tế và bàn giao trace.
